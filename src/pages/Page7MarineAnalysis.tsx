...
export const Page7MarineAnalysis: React.FC = () => {
  ...
  return (
    <div className="w-full px-4 py-5 sm:px-8 space-y-6 select-none">
      ...
      <div className="bg-ocean-card p-5 rounded-2xl border border-ocean-sub shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <div className="flex items-center space-x-2">
              <Activity className="w-5 h-5 text-[#2a8a89]" />
          ...
          <div className="flex items-center space-x-2 self-start sm:self-auto">
            <Anchor className="w-4 h-4 text-[#2a8a89]" />
            <select
              ...
              className="px-3 py-1.5 bg-ocean-sub/40 border border-ocean-sub rounded-xl text-xs font-semibold text-ocean-text"
            >
            ...
            return (
              <button
                ...
                className={`flex items-center space-x-2 p-3 rounded-xl border text-xs font-bold transition-all ${
                  isSelected
                    ? 'bg-[#2a8a89] text-white border-[#2a8a89] shadow-xs'
                    : 'bg-ocean-card text-ocean-text/90 border-ocean-sub hover:bg-[#c8dcdb] hover:border-[#2a8a89]/40'
                }`}
              >
                <Icon className="w-4 h-4 shrink-0" />
                <div className="text-left truncate">
                  <div>{cat.label}</div>
                  <div className={`text-[10px] font-normal truncate ${isSelected ? 'text-white/80' : 'text-ocean-text/50'}`}>
                    {cat.desc.split('(')[0]}
                  </div>
                </div>
              </button>
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          <div className="bg-ocean-card p-4 rounded-2xl border border-ocean-sub shadow-xs space-y-3">
            <div className="flex items-center justify-between border-b border-ocean-sub/60 pb-2">
              ...
              <span className="text-[10px] text-ocean-text/50">Sensor Telemetry</span>
            </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                {activeCategory === 'SST' ? (
                  <LineChart data={SST_CHART_DATA} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <Line type="monotone" dataKey="depthTemp" stroke="#2a8a89" strokeWidth={1.8} strokeDasharray="4 4" name="10m Depth (°C)" />
                  </LineChart>
            ...
            {/* Key Metric Tags */}
            <div className="grid grid-cols-3 gap-2 pt-1 border-t border-ocean-sub/60">
              {Object.entries(aiInsight.metrics).map(([key, val]) => (
                <div key={key} className="p-2 bg-ocean-sub/40 rounded-lg text-center">
                  <div className="text-[10px] text-ocean-text/70 truncate">{key}</div>
                  <div className="text-xs font-bold text-ocean-text mt-0.5">{val}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Top Risk Zones List */}
          <div className="bg-ocean-card p-4 rounded-2xl border border-ocean-sub shadow-xs space-y-2.5">
            <div className="flex items-center justify-between border-b border-ocean-sub/60 pb-2">
              <div className="flex items-center space-x-1.5 text-xs font-bold text-red-600 uppercase">
                <AlertTriangle className="w-3.5 h-3.5" />
                <span>{t.topRiskZonesTitle}</span>
              </div>
              <span className="text-[10px] text-ocean-text/50">Hazard Matrix</span>
            </div>

            {riskZones.slice(0, 2).map(rz => (
              <div key={rz.id} className="p-2.5 bg-ocean-sub/40 rounded-xl border border-ocean-sub/60 text-xs">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-ocean-text">{rz.name}</span>
                  <span className="text-[10px] font-bold text-red-600 bg-red-50 px-1.5 py-0.5 rounded">
                    {rz.riskScore}/100
                  </span>
                </div>
                <div className="text-[10px] text-ocean-text/70 mt-1 italic">
                  {rz.advisory}
                </div>
              </div>
            ))}
