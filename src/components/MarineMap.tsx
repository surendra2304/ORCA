...
export const MarineMap: React.FC<MarineMapProps> = ({
  ...
  useEffect(() => {
    ...
    const userPinHtml = `
      <div class="relative flex items-center justify-center -translate-x-1/2 -translate-y-full cursor-pointer group">
        ...
        <span class="absolute -bottom-5 bg-ocean-card text-ocean-text text-[10px] font-bold px-1.5 py-0.5 rounded shadow border border-ocean-sub whitespace-nowrap">
    ...
    userMarker.bindPopup(`
      <div class="text-xs p-1">
        <div class="font-bold" style="color:#0f2a30">${t.currentLocationLabel}</div>
        <div style="color:#0f2a3099">${selectedRegion.name}</div>
        <div class="text-[11px] font-semibold mt-1" style="color:#2a8a89">Lat: ${selectedRegion.userBase.lat.toFixed(2)}, Lng: ${selectedRegion.userBase.lng.toFixed(2)}</div>
      </div>
    `);
    ...
    activeZonesToRender.forEach((zone) => {
      ...
      const fishSvg = `
        <svg viewBox="0 0 24 24" width="22" height="22" fill="white" stroke="white" stroke-width="1.2">
          ...
          <circle cx="16" cy="10.5" r="1" fill="#2a8a89"/>
        </svg>
      `;
      ...
      zoneMarker.bindPopup(`
        <div class="p-2 min-w-[200px] text-xs font-sans">
          <div class="flex items-center justify-between border-b pb-1 mb-1.5">
            <span class="font-bold text-ocean-text">${zone.code}</span>
          ...
          <div class="text-ocean-text font-medium">${zone.name}</div>
          <div class="mt-1 text-ocean-text/80 grid grid-cols-2 gap-1 text-[11px]">
          ...
          <div class="mt-2 text-[10px] text-ocean-text/70 italic bg-slate-50 p-1.5 rounded border border-slate-100">
            ${zone.notes}
          </div>
        </div>
      `);
        ...
        if (risk.regionId === selectedRegion.id) {
          ...
          riskMarker.bindPopup(`
            <div class="p-2 min-w-[210px] text-xs font-sans">
              ...
              <div class="font-bold text-ocean-text">${risk.name}</div>
              <div class="text-ocean-text/80 mt-1 text-[11px]">${risk.hazardType}</div>
  ...
  return (
    <div className={`relative w-full ${customHeight} bg-ocean-sub/50 rounded-xl overflow-hidden border border-ocean-sub shadow-xs select-none`}>
      {/* Map DOM Container */}
      ...
      <div className="absolute top-4 right-4 z-10 flex flex-col space-y-2">
        ...
        <button
          ...
          className="w-9 h-9 bg-ocean-card/95 hover:bg-ocean-card text-ocean-text/90 hover:text-[#2a8a89] rounded-lg shadow-md border border-ocean-sub/80 flex items-center justify-center transition-all active:scale-95"
          title="Zoom In"
        >
        ...
        <div className="relative">
          <button
            ...
            className={`w-9 h-9 ${
              showLayerMenu ? 'bg-ocean-primary text-white' : 'bg-ocean-card/95 hover:bg-ocean-card text-ocean-text/90 hover:text-ocean-primary'
            } rounded-lg shadow-md border border-ocean-sub/80 flex items-center justify-center transition-all active:scale-95`}
          ...
          {showLayerMenu && (
            <div className="absolute right-11 top-0 w-48 bg-ocean-card rounded-xl shadow-xl border border-ocean-sub py-1.5 z-20 text-xs font-medium">
              <div className="px-3 py-1 text-[10px] uppercase font-bold text-ocean-text/50 tracking-wider">
                {t.navAnalysis}
              </div>
              {[
                ...
                <button
                  ...
                  className={`w-full px-3 py-1.5 flex items-center justify-between text-left hover:bg-ocean-sub/40 transition-colors ${
                    currentLayer === layer.id ? 'text-ocean-primary font-bold bg-ocean-sub/50' : 'text-ocean-text/90'
                  }`}
                >
                  <span className="truncate">{layer.label}</span>
                  {currentLayer === layer.id && <Check className="w-3.5 h-3.5 text-ocean-primary shrink-0" />}
                </button>
              ))}
            </div>
          )}
      ...
      <div className="absolute bottom-3 left-3 z-10 bg-ocean-card/95 backdrop-blur-xs px-3 py-1.5 rounded-lg shadow-xs border border-ocean-sub/90 text-xs flex items-center space-x-3">
        <div className="flex items-center space-x-1.5">
          <span className="w-2 h-2 rounded-full bg-ocean-primary"></span>
          <span className="font-semibold text-ocean-text">{selectedRegion.name}</span>
        </div>
        <div className="text-ocean-text/40">|</div>
        <div className="text-[11px] text-ocean-text/80 font-medium">
