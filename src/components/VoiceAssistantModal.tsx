...
export const VoiceAssistantModal: React.FC<VoiceAssistantModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-4 animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg bg-ocean-card rounded-2xl shadow-2xl border border-ocean-sub/60 overflow-hidden">
        
        {/* Header with Close / X to switch to manual input */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-ocean-sub/60 bg-ocean-sub/40/50">
          <div className="flex items-center space-x-2">
            <div className="w-8 h-8 rounded-full overflow-hidden bg-ocean-card border border-ocean-sub flex items-center justify-center">
              <img src="/orca-logo.jpg" alt="ORCA" className="w-full h-full object-contain" />
            </div>
            <div>
              <h3 className="text-sm font-bold text-ocean-text">{t.voiceAssistantTitle}</h3>
              <p className="text-[11px] text-[#2a8a89] font-medium">{t.voiceListeningStatus}</p>
            </div>
          </div>

          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-ocean-text/50 hover:text-ocean-text/90 hover:bg-slate-100 transition-colors"
        ...
        <div className="p-6 text-center">
          ...
          <div className="relative inline-flex items-center justify-center my-3">
            {isListening && (
              <>
                <span className="absolute w-24 h-24 rounded-full bg-[#2a8a89]/15 animate-ping"></span>
                <span className="absolute w-20 h-20 rounded-full bg-[#2a8a89]/25 animate-pulse"></span>
              </>
            )}
            <button
              onClick={() => setIsListening(!isListening)}
              className="relative z-10 w-16 h-16 rounded-full bg-[#2a8a89] text-white flex items-center justify-center shadow-lg hover:bg-[#38a3a5] transition-all"
            >
          ...
          <div className="min-h-[56px] flex items-center justify-center mb-6">
            <p className="text-base sm:text-lg font-semibold text-ocean-text leading-snug">
              {speechFeedback}
            </p>
          </div>

          {/* Sequential Step Interactive Answer Chips */}
          <div className="bg-ocean-sub/40 p-4 rounded-xl border border-ocean-sub/60 mb-4 text-left">
            {currentStep === 0 && (
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {COASTAL_REGIONS.map(reg => (
                    <button
                      ...
                      className="px-3 py-2 text-xs font-medium text-ocean-text bg-ocean-card hover:bg-[#c8dcdb] hover:text-[#2a8a89] hover:border-[#2a8a89] rounded-lg border border-ocean-sub transition-colors text-left"
                    >
                      {reg.name}
                    </button>
                  ))}
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <button
                      ...
                      className="px-3 py-2 text-xs font-medium text-ocean-text bg-ocean-card hover:bg-[#c8dcdb] hover:text-[#2a8a89] hover:border-[#2a8a89] rounded-lg border border-ocean-sub transition-colors"
                    >
                      {item.label}
                    </button>
                  ))}
          ...
          <div className="flex items-center justify-between pt-2">
            <button
              onClick={onClose}
              className="text-xs text-ocean-text/70 hover:text-ocean-text font-medium underline underline-offset-4"
            >
            ...
            {currentStep === 4 ? (
              <button
                ...
                className="flex items-center space-x-2 px-5 py-2.5 bg-[#2a8a89] hover:bg-[#38a3a5] text-white text-xs font-bold rounded-xl shadow-md transition-all active:scale-95"
              >
