...
async def _async_test_event_chronology():
    ...
    # 1 run_started + 1 plan_created + 2*N agent events + 1 reflection + 1 verdict + 1 final_answer + 1 run_complete
    expected_length = 1 + 1 + 2 * n_agents + 1 + 1 + 1 + 1
    assert (
    ...
    assert trace[-4]["event"] == "reflection", f"Pre-verdict event must be reflection, got {trace[-4]['event']}"
    assert trace[-3]["event"] == "verdict", f"Antepenultimate event must be verdict, got {trace[-3]['event']}"
    assert trace[-2]["event"] == "final_answer", f"Penultimate event must be final_answer, got {trace[-2]['event']}"
