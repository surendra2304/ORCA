...
def build_graph(collector: TraceCollector):
    ...
    START -> planner -> resolver -> executor -> reflection -> (sufficient? verdict : replan -> executor_replan -> verdict) -> aggregator -> END.
    Node functions close over the per-run TraceCollector instance.
    Guaranteed structural single-invocation of reflection and replan (bounded reflection DAG).
    """
    ...
    async def _reflection_node(state: ORCAState) -> Dict[str, Any]:
        return await reflection_node(state, collector)

    async def _replan_node(state: ORCAState) -> Dict[str, Any]:
        return await replan_node(state, collector)

    async def _executor_replan_node(state: ORCAState) -> Dict[str, Any]:
        return await executor_replan_node(state, collector)
    ...
    builder.add_node("executor_replan", _executor_replan_node)
    ...
    builder.add_edge("replan", "executor_replan")
    builder.add_edge("executor_replan", "verdict")
